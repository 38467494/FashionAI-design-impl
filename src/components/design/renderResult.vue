  ...
  methods: {
    ...
    submit: function () {
      ...
      Render({
        ...
        var renderResult = res.data.data.fileUrl
        this.resultImage = renderResult
    ...
    collect: async function () {
      ...
      for (var i = 0; i < this.selectList.length; i++) {
        ...
        var cover = await getCoverImg("render", workDescription)
        //因为当前的color和result的url是有过期时间的，加入需要收藏，要重新上传图片以长期保存
        workDescription.color = await uploadURL("render", workDescription.color)
        workDescription.result = await uploadURL("render", workDescription.result)
...
<style scoped>
@import "../../assets/css/design/renderResult.css";
